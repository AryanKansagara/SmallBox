...
export default function FinancePage() {
  ...
  // Area chart: monthly aggregation
  const monthlyData = useMemo(() => {
    const months = ["2026-01", "2026-02", "2026-03", "2026-04", "2026-05"];
    // Fill with seed base for earlier months that have no transactions
  ...
  // Pie / bar chart: expense breakdown by category (current month)
  ...
  // Budget used amounts (current month)
  ...
  const saveBudget = () => {
    ...
    const COLORS: Record<string, string> = {
      Supplies: "#0062ff", Marketing: "#10b981", Utilities: "#f59e0b", Rent: "#8b5cf6", Other: "#38bdf8",
    };
    setBudgets((prev) => {
      const existing = prev.find((b) => b.category === budgetForm.category);
      if (existing) {
        return prev.map((b) => b.category === budgetForm.category ? { ...b, total } : b);
      }
    ...
    <div className="flex flex-col flex-1">
      ...
      <main className="flex-1 p-6 space-y-6">
          ...
          {activeTab === "overview" && (
            <motion.div key="overview" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              ...
              <div className="rounded-2xl border border-[#2a3a55] bg-[#111827] p-6">
                ...
                <p className="text-xs text-[#4b5e7a] mb-4">By category · May 2026</p>
            ...
            <motion.div key="transactions" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="rounded-2xl border border-[#2a3a55] bg-[#111827] overflow-hidden">
              <div className="flex items-center justify-between p-5 border-b border-[#2a3a55] flex-wrap gap-3">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#2a3a55] text-xs text-[#8b9cb6] hover:text-white hover:border-[#0062ff]/30 transition-all">
              ...
              {filteredTxns.length === 0 ? (
                <div className="py-16 text-center text-[#4b5e7a] text-sm">No transactions found. Add one above.</div>
              ) : (
            ...
            <motion.div key="budgets" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              ...
              <div className="rounded-2xl border border-[#2a3a55] bg-[#111827] p-6">
                <div className="flex items-center justify-between mb-5">
                  <div>
                    <h3 className="font-semibold text-white">May 2026 Budget</h3>
                    <p className="text-xs text-[#4b5e7a] mt-0.5">Click a category to edit its limit</p>
                  </div>
                  <motion.button
                    whileHover={{ scale: 1.04 }}
                    whileTap={{ scale: 0.96 }}
                          ...
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                ...
                                className="w-5 h-5 rounded flex items-center justify-center text-[#4b5e7a] hover:text-[#0062ff] transition-colors"
                              >
                                <Pencil size={11} />
                              </button>
                            </div>
              ...
              <div className="rounded-2xl border border-[#2a3a55] bg-[#111827] p-6">
                ...
                <p className="text-xs text-[#4b5e7a] mb-5">May 2026 — live from your transactions</p>
                {expenseByCategory.length === 0 ? (
        ...
        <motion.div
          ...
          className="rounded-2xl border-2 border-dashed border-[#2a3a55] hover:border-[#0062ff]/40 transition-colors p-8 text-center group cursor-pointer"
        >
          <Upload size={28} className="text-[#4b5e7a] group-hover:text-[#0062ff] mx-auto mb-3 transition-colors" />
          <p className="font-medium text-[#8b9cb6] group-hover:text-white transition-colors">Upload Receipt or Bank Statement</p>
          <p className="text-sm text-[#4b5e7a] mt-1">Watson AI will extract and categorize transactions automatically</p>
          <p className="text-xs text-[#4b5e7a] mt-2">CSV, PDF, or Image — up to 10MB</p>
        </motion.div>
      </main>
              ...
              <div className="bg-[#111827] border border-[#2a3a55] rounded-2xl p-6 shadow-2xl">
                ...
                <div className="space-y-4">
                    ...
                    {(["income", "expense"] as const).map((t) => (
                      <button
                        ...
                        className={`py-2 rounded-xl text-sm font-medium capitalize border transition-all ${newEntry.type === t
                          ? t === "income" ? "bg-[#10b981]/15 border-[#10b981]/40 text-[#10b981]" : "bg-[#ef4444]/15 border-[#ef4444]/40 text-[#ef4444]"
                  ...
                  <input
                    ...
                    onChange={(e) => { setNewEntry({ ...newEntry, description: e.target.value }); setEntryError(""); }}
                    className="w-full bg-[#1a2235] border border-[#2a3a55] focus:border-[#0062ff]/50 rounded-xl px-4 py-2.5 text-white text-sm placeholder:text-[#4b5e7a] outline-none"
                  />

                  <input
                    placeholder="Amount (e.g. 234.50) *"
                    value={newEntry.amount}
                    onChange={(e) => { setNewEntry({ ...newEntry, amount: e.target.value }); setEntryError(""); }}
                    type="number"
                    min="0"
                    step="0.01"
                    className="w-full bg-[#1a2235] border border-[#2a3a55] focus:border-[#0062ff]/50 rounded-xl px-4 py-2.5 text-white text-sm placeholder:text-[#4b5e7a] outline-none"
                ...
                <div className="flex gap-3 mt-6">
                  <button onClick={() => setShowAddModal(false)} className="flex-1 py-2.5 rounded-xl border border-[#2a3a55] text-[#8b9cb6] text-sm hover:text-white transition-colors">
                    Cancel
                  </button>
                  <motion.button
                    ...
                    className="flex-1 py-2.5 rounded-xl bg-[#0062ff] text-white text-sm font-semibold flex items-center justify-center gap-1.5"
                  >
        ...
        {showBudgetModal && (
          <>
            ...
            >
              <div className="bg-[#111827] border border-[#2a3a55] rounded-2xl p-6 shadow-2xl">
                <div className="flex items-center justify-between mb-5">
                  <h3 className="text-lg font-semibold text-white">{editingBudget ? `Edit ${editingBudget.category} Budget` : "Set Budget"}</h3>
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      min="0"
                      step="1"
                      className="w-full bg-[#1a2235] border border-[#2a3a55] focus:border-[#0062ff]/50 rounded-xl pl-8 pr-4 py-2.5 text-white text-sm outline-none"
                    />
                  </div>
                </div>

                <div className="flex gap-3 mt-6">
                  <button onClick={() => setShowBudgetModal(false)} className="flex-1 py-2.5 rounded-xl border border-[#2a3a55] text-[#8b9cb6] text-sm hover:text-white transition-colors">
                    Cancel
                  </button>
                  <motion.button
                    whileHover={{ scale: 1.03 }}
