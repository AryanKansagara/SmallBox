...
const CAT_COLORS: Record<string, string> = {
  Supplies: "#7b2fff", Marketing: "#10b981", Utilities: "#f59e0b",
...
// Recharts tooltip style — uses CSS vars so it adapts to light/dark
const tooltipStyle = {
      ...
      <main className="flex-1 p-6 space-y-6">
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                ...
                { label: "Net Profit", value: `$${netProfit.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, icon: netProfit >= 0 ? ArrowUpRight : ArrowDownRight, color: netProfit >= 0 ? "text-primary" : "text-[#ef4444]", bg: netProfit >= 0 ? "bg-primary/10" : "bg-[#ef4444]/10", border: netProfit >= 0 ? "border-primary/20" : "border-[#ef4444]/20" },
              ].map((s, i) => (
                <motion.div key={s.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} whileHover={{ y: -3 }}
                  className={`rounded-2xl border ${s.border} glass-card p-5`}>
            ...
            {/* Tabs */}
            <div className="flex items-center gap-1 p-1 bg-black/5 dark:bg-white/5 rounded-xl border border-black/8 dark:border-white/8 w-fit flex-wrap backdrop-blur-sm">
            ...
            <AnimatePresence mode="wait">
                ...
                <motion.div key="overview" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="space-y-6">
                  <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                    <div className="xl:col-span-2 rounded-2xl border border-black/8 dark:border-white/8 glass-card p-6">
                      ...
                      {monthlyData.length === 0 ? (
                        <div className="flex items-center justify-center h-60 text-black/40 dark:text-white/40 text-sm">Add transactions to see the trend</div>
                      ) : (
                    ...
                    <div className="rounded-2xl border border-black/8 dark:border-white/8 glass-card p-6">
                      ...
                      <p className="text-xs text-black/40 dark:text-white/40 mb-4">By category · {currentMonthDisplay}</p>
                      {expenseByCategory.length === 0 ? (
                        <div className="flex items-center justify-center h-48 text-black/40 dark:text-white/40 text-sm">No expenses yet</div>
                      ) : (
                        <>
                  ...
                  <div className="rounded-2xl border border-black/8 dark:border-white/8 glass-card p-6">
                    <div className="flex items-center justify-between flex-wrap gap-3">
                      <div>
                        <h3 className="font-semibold text-[#1d1d1f] dark:text-white flex items-center gap-2">
                          <FileText size={16} className="text-primary" />
                          Monthly Summary Report
                        </h3>
                        <p className="text-xs text-black/40 dark:text-white/40 mt-0.5">AI-generated business summary · Powered by watsonx.ai</p>
                      </div>
                      <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={generateReport} disabled={reportLoading}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary text-white text-xs font-semibold disabled:opacity-60">
                        {reportLoading ? <Loader2 size={12} className="animate-spin" /> : <Sparkles size={12} />}
                        {reportLoading ? "Generating…" : "Generate Report"}
                      </motion.button>
                    </div>
                    {report ? (
                      <div className="mt-4 rounded-xl bg-black/4 dark:bg-white/4 border border-black/8 dark:border-white/8 p-4">
                        <p className="text-sm text-[#1d1d1f] dark:text-white/85 leading-relaxed">{report}</p>
                        <div className="mt-3 flex items-center gap-1.5 text-[10px] text-black/40 dark:text-white/40">
                          <Sparkles size={9} className="text-primary" />
                          {reportSource}
                        </div>
                      </div>
                    ) : !reportLoading ? (
                      <p className="text-xs text-black/40 dark:text-white/40 mt-4">Click &quot;Generate Report&quot; to get a plain-language AI summary of this month&apos;s performance.</p>
                    ) : null}
                  </div>
                </motion.div>
              )}
              ...
              {activeTab === "transactions" && (
                <motion.div key="transactions" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}
                  className="rounded-2xl border border-black/8 dark:border-white/8 glass-card overflow-hidden">
                      ...
                      <div className="flex items-center gap-1 p-0.5 bg-black/5 dark:bg-white/5 rounded-lg border border-black/8 dark:border-white/8">
                        {(["all", "income", "expense"] as const).map((f) => (
                          <button key={f} onClick={() => setFilterType(f)}
                            className={`px-2.5 py-1 rounded-md text-xs font-medium capitalize transition-all ${filterType === f ? "bg-primary text-white" : "text-black/50 dark:text-white/50 hover:text-black dark:hover:text-white"}`}>
                            {f}
                          </button>
                        ))}
                      </div>
                      <button onClick={exportCsv}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-black/8 dark:border-white/8 text-xs text-black/50 dark:text-white/50 hover:text-black dark:hover:text-white hover:border-primary/30 transition-all">
                  ...
                  {filteredTxns.length === 0 ? (
                    <div className="py-16 text-center text-black/40 dark:text-white/40 text-sm">No transactions yet. Add one or upload a bank statement below.</div>
                  ) : (
                    <div className="divide-y divide-black/5 dark:divide-white/5 max-h-[480px] overflow-y-auto">
                      {filteredTxns.map((t, i) => (
                        <motion.div key={t.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: Math.min(i, 15) * 0.03 }}
                          className="flex items-center justify-between px-5 py-3.5 hover:bg-black/3 dark:hover:bg-white/4 transition-colors group">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => deleteTransaction(t.id)}
                              className="opacity-0 group-hover:opacity-100 w-6 h-6 rounded-lg hover:bg-[#ef4444]/10 flex items-center justify-center text-black/30 dark:text-white/30 hover:text-[#ef4444] transition-all">
                              <X size={12} />
                            </motion.button>
                          </div>
                        </motion.div>
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                    <div className="rounded-2xl border border-black/8 dark:border-white/8 glass-card p-6">
                      <div className="flex items-center justify-between mb-5">
                        <div>
                          <h3 className="font-semibold text-[#1d1d1f] dark:text-white">{currentMonthDisplay} Budget</h3>
                          <p className="text-xs text-black/40 dark:text-white/40 mt-0.5">Click pencil to edit a limit</p>
                        </div>
                      ...
                      {budgets.length === 0 ? (
                        <p className="text-center text-black/40 dark:text-white/40 text-sm py-10">No budgets set yet. Add one above.</p>
                      ) : (
                              ...
                              <div key={b.category}>
                                <div className="flex items-center justify-between mb-2">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <button onClick={() => { setEditingBudget(b); setBudgetForm({ category: b.category, total: String(b.total) }); setShowBudgetModal(true); }}
                                      className="w-5 h-5 rounded flex items-center justify-center text-black/30 dark:text-white/30 hover:text-primary transition-colors">
                                      <Pencil size={11} />
                                    </button>
                                    <button onClick={() => deleteBudget(b.category)}
                                      className="w-5 h-5 rounded flex items-center justify-center text-black/30 dark:text-white/30 hover:text-[#ef4444] transition-colors">
                                      <X size={11} />
                                    </button>
                                  </div>
                                ...
                                <div className="h-2.5 bg-black/8 dark:bg-white/8 rounded-full overflow-hidden">
                                  <motion.div key={`${b.category}-${used}`} initial={{ width: 0 }} animate={{ width: `${Math.min(pct, 100)}%` }} transition={{ duration: 0.8, ease: "easeOut" }}
                                    className="h-full rounded-full" style={{ background: barColor }} />
                                </div>
                                <div className="flex justify-between mt-1">
                                  <span className="text-[10px] text-black/40 dark:text-white/40">{pct}% used</span>
                    ...
                    <div className="rounded-2xl border border-black/8 dark:border-white/8 glass-card p-6">
                      ...
                      <p className="text-xs text-black/40 dark:text-white/40 mb-5">{currentMonthDisplay} · live from your transactions</p>
                      {expenseByCategory.length === 0 ? (
                        <div className="flex items-center justify-center h-64 text-black/40 dark:text-white/40 text-sm">No expense data yet</div>
                      ) : (
                        <ResponsiveContainer width="100%" height={260}>
                          <BarChart data={expenseByCategory} margin={{ top: 4, right: 4, left: -24, bottom: 0 }}>
                            ...
                            <YAxis tick={{ fontSize: 11 }} axisLine={false} tickLine={false} />
                            <Tooltip contentStyle={tooltipStyle} formatter={(v) => [`$${Number(v).toFixed(2)}`, "Spent"]} />
                  ...
                  <div className="rounded-2xl border border-black/8 dark:border-white/8 glass-card p-6">
                    <div className="flex items-center justify-between flex-wrap gap-3">
                      <div>
                        ...
                        <p className="text-xs text-black/40 dark:text-white/40 mt-0.5">Actionable recommendations · Powered by watsonx.ai</p>
                      </div>
                    ...
                    {showInsights && (
                      <div className="mt-4 space-y-3">
                        {insightsLoading ? (
                          <div className="py-6 flex items-center justify-center gap-2 text-black/40 dark:text-white/40 text-sm">
                            <Loader2 size={16} className="animate-spin" /> Analysing your budget…
                          </div>
                        ) : insights.length === 0 ? (
                          <p className="text-sm text-black/40 dark:text-white/40">No insights could be generated — try adding budgets and transactions first.</p>
                        ) : (
                          <>
                            {insights.map((ins, i) => (
                              <motion.div key={i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}
                                className={`rounded-xl p-3 flex gap-3 ${ins.type === "warning" ? "bg-[#ef4444]/10 border border-[#ef4444]/20" : ins.type === "positive" ? "bg-[#10b981]/10 border border-[#10b981]/20" : "bg-primary/10 border border-primary/20"}`}>
                                <div className="mt-0.5">
                                  {ins.type === "warning" ? <AlertTriangle size={14} className="text-[#ef4444]" /> : ins.type === "positive" ? <TrendingUp size={14} className="text-[#10b981]" /> : <Lightbulb size={14} className="text-primary" />}
                                </div>
                                <div>
                                  <p className="text-sm font-semibold text-[#1d1d1f] dark:text-white">{ins.title}</p>
                                  <p className="text-xs text-black/50 dark:text-white/50 mt-0.5 leading-relaxed">{ins.body}</p>
                                </div>
                              </motion.div>
                            ))}
                            <div className="flex items-center gap-1.5 text-[10px] text-black/40 dark:text-white/40 pt-1">
                              <Sparkles size={9} className="text-primary" />
                              {insightsSource}
                            </div>
                          </>
              ...
              {activeTab === "analytics" && (
                <motion.div key="analytics" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="font-semibold text-[#1d1d1f] dark:text-white flex items-center gap-2">
                        <BarChart2 size={16} className="text-primary" /> Business Analytics
                      </h3>
                      <p className="text-xs text-black/40 dark:text-white/40 mt-0.5">Powered by IBM Cloud SQL Query · each card shows the underlying SQL</p>
                    </div>
                    <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={fetchAnalytics} disabled={analyticsLoading}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-black/8 dark:border-white/8 text-xs text-black/50 dark:text-white/50 hover:text-black dark:hover:text-white hover:border-primary/30 transition-all disabled:opacity-50">
                  ...
                  {analyticsLoading ? (
                    <div className="py-16 flex items-center justify-center gap-2 text-black/40 dark:text-white/40 text-sm">
                      <Loader2 size={20} className="animate-spin" /> Running analytics queries…
                    </div>
                  ) : analytics.length === 0 ? (
                    <div className="py-16 text-center text-black/40 dark:text-white/40 text-sm">No data yet — add some transactions to see analytics.</div>
                  ) : (
                    <>
                      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                        {analytics.map((a, i) => (
                          <div key={i} className="rounded-2xl border border-black/8 dark:border-white/8 glass-card p-5">
                            <p className="text-xs text-black/40 dark:text-white/40 uppercase tracking-wide mb-1">{a.label}</p>
                            <p className="text-lg font-bold text-[#1d1d1f] dark:text-white">{a.value}</p>
                            {a.detail && <p className="text-xs text-black/50 dark:text-white/50 mt-0.5">{a.detail}</p>}
                            <button onClick={() => setExpandedSql(expandedSql === i ? null : i)}
                              className="mt-3 flex items-center gap-1 text-[10px] text-black/40 dark:text-white/40 hover:text-primary transition-colors">
                            ...
                            <AnimatePresence>
                              {expandedSql === i && (
                                <motion.pre initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                                  className="mt-2 overflow-hidden text-[10px] text-black/50 dark:text-white/50 bg-black/4 dark:bg-white/4 rounded-lg p-3 font-mono leading-relaxed whitespace-pre-wrap">
                                  {a.sql}
                                </motion.pre>
                              )}
                            </AnimatePresence>
                      ...
                      <div className="rounded-xl border border-black/8 dark:border-white/8 p-3 flex items-start gap-2 text-xs text-black/40 dark:text-white/40">
                        <BarChart2 size={12} className="text-primary mt-0.5 shrink-0" />
                        <span>
                          <span className="text-black/60 dark:text-white/60 font-medium">{analyticsSource}</span>
                          {" — Add "}
                          <code className="text-primary">IBM_SQL_QUERY_CRN</code>
                          {" + "}
                          <code className="text-primary">IBM_COS_BUCKET</code>
                          {" env vars to run these queries against IBM Cloud SQL Query."}
                        </span>
                      </div>
                    </>
                  )}
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div
            ...
            className="rounded-2xl border-2 border-dashed border-black/10 dark:border-white/10 hover:border-primary/40 transition-colors p-6 text-center group cursor-pointer">
            <input ref={fileInputRef} type="file" accept=".csv,text/csv" className="hidden"
              onChange={(e) => { const f = e.target.files?.[0]; if (f) void handleFileUpload(f); e.target.value = ""; }} />
            <Upload size={24} className="text-black/30 dark:text-white/30 group-hover:text-primary mx-auto mb-2 transition-colors" />
            <p className="font-medium text-black/55 dark:text-white/55 group-hover:text-[#1d1d1f] dark:group-hover:text-white transition-colors text-sm">Upload Bank Statement CSV</p>
            <div className="flex items-center justify-center gap-1 mt-1">
              <Sparkles size={10} className="text-primary" />
              <p className="text-xs text-black/40 dark:text-white/40">AI categorizes each row — review before importing</p>
            </div>
          </div>
          ...
          <div
            ...
            className="rounded-2xl border-2 border-dashed border-black/10 dark:border-white/10 hover:border-primary/40 transition-colors p-6 text-center group cursor-pointer">
            <Camera size={24} className="text-black/30 dark:text-white/30 group-hover:text-primary mx-auto mb-2 transition-colors" />
            <p className="font-medium text-black/55 dark:text-white/55 group-hover:text-[#1d1d1f] dark:group-hover:text-white transition-colors text-sm">Scan Receipt</p>
            <div className="flex items-center justify-center gap-1 mt-1">
              <ScanLine size={10} className="text-primary" />
              <p className="text-xs text-black/40 dark:text-white/40">Camera or photo — OCR runs on-device</p>
            </div>
          </div>
        </motion.div>
      </main>
        ...
        {showReviewModal && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              onClick={() => { if (!importingRows) { setShowReviewModal(false); setReviewRows([]); } }}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
              ...
              <div className="glass-strong border border-black/8 dark:border-white/8 rounded-2xl shadow-2xl flex flex-col overflow-hidden">
                <div className="flex items-center justify-between px-6 py-4 border-b border-black/8 dark:border-white/8">
                  <div>
                    <h3 className="text-base font-semibold text-[#1d1d1f] dark:text-white">Review & confirm import</h3>
                    <p className="text-xs text-black/40 dark:text-white/40 mt-0.5 flex items-center gap-1">
                      <Sparkles size={10} className="text-primary" />
                      watsonx.ai categorized each transaction — edit any row before importing
                    </p>
                  </div>
                  <button onClick={() => { if (!importingRows) { setShowReviewModal(false); setReviewRows([]); } }}
                    className="text-black/40 dark:text-white/40 hover:text-black dark:hover:text-white transition-colors"><X size={18} /></button>
                </div>

                <div className="overflow-y-auto flex-1">
                  <table className="w-full text-sm">
                    <thead className="sticky top-0 bg-black/4 dark:bg-white/4 border-b border-black/8 dark:border-white/8">
                      <tr>
                        <th className="text-left px-4 py-2.5 text-xs text-black/40 dark:text-white/40 font-medium">Description</th>
                        <th className="text-left px-4 py-2.5 text-xs text-black/40 dark:text-white/40 font-medium">Category</th>
                        <th className="text-left px-4 py-2.5 text-xs text-black/40 dark:text-white/40 font-medium">Type</th>
                        <th className="text-right px-4 py-2.5 text-xs text-black/40 dark:text-white/40 font-medium">Amount</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-black/5 dark:divide-white/5">
                      {reviewRows.map((row, i) => (
                        <tr key={i} className="hover:bg-black/3 dark:hover:bg-white/4 transition-colors">
                          <td className="px-4 py-2.5 text-[#1d1d1f] dark:text-white max-w-[220px] truncate">{row.description}</td>
                          <td className="px-4 py-2.5">
                            {row.categorizing ? (
                              <div className="flex items-center gap-1.5 text-black/40 dark:text-white/40 text-xs">
                                <Loader2 size={11} className="animate-spin" /> Asking AI…
                              </div>
                            ) : (
                              <div className="flex items-center gap-1.5">
                                <select value={row.category}
                                  onChange={(e) => setReviewRows((prev) => prev.map((r, idx) => idx === i ? { ...r, category: e.target.value } : r))}
                                  className="bg-black/4 dark:bg-white/4 border border-black/10 dark:border-white/10 rounded-lg px-2 py-1 text-[#1d1d1f] dark:text-white text-xs outline-none focus:border-primary/50 w-full max-w-[140px]">
                                  {CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
                                </select>
                                {row.source !== "pending" && (
                                  <span title={`Categorized by ${row.source === "watsonx" ? "watsonx.ai" : "local classifier"}`}>
                                    <Sparkles size={10} className={row.source === "watsonx" ? "text-primary" : "text-black/30 dark:text-white/30"} />
                                  </span>
                                )}
                              </div>
                            )}
                            ...
                            <div className="flex gap-1">
                              {(["expense", "income"] as const).map((t) => (
                                <button key={t} onClick={() => setReviewRows((prev) => prev.map((r, idx) => idx === i ? { ...r, type: t } : r))}
                                  className={`px-2 py-0.5 rounded text-[10px] font-medium capitalize border transition-all ${row.type === t ? (t === "income" ? "bg-[#10b981]/15 border-[#10b981]/40 text-[#10b981]" : "bg-[#ef4444]/15 border-[#ef4444]/40 text-[#ef4444]") : "bg-black/4 dark:bg-white/4 border-black/10 dark:border-white/10 text-black/40 dark:text-white/40"}`}>
                                  {t}
                                </button>
                              ))}
                            </div>
                          </td>
                          <td className="px-4 py-2.5 text-right font-semibold text-[#1d1d1f] dark:text-white">${row.amount.toFixed(2)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="px-6 py-4 border-t border-black/8 dark:border-white/8 flex items-center justify-between gap-3">
                  {importDone ? (
                    ...
                    <p className="text-xs text-black/40 dark:text-white/40">{reviewRows.filter((r) => !r.categorizing).length}/{reviewRows.length} categorized</p>
                  )}
                  <div className="flex gap-3">
                    <button onClick={() => { setShowReviewModal(false); setReviewRows([]); }} disabled={importingRows}
                      className="px-4 py-2 rounded-xl border border-black/10 dark:border-white/10 text-black/50 dark:text-white/50 text-sm hover:text-black dark:hover:text-white transition-colors disabled:opacity-50">
        ...
        {showAddModal && (
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
              ...
              <div className="glass-strong border border-black/8 dark:border-white/8 rounded-2xl p-6 shadow-2xl">
                <div className="flex items-center justify-between mb-5">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-2">
                    {(["income", "expense"] as const).map((t) => (
                      <button key={t} onClick={() => setNewEntry({ ...newEntry, type: t })}
                        className={`py-2 rounded-xl text-sm font-medium capitalize border transition-all ${newEntry.type === t ? (t === "income" ? "bg-[#10b981]/15 border-[#10b981]/40 text-[#10b981]" : "bg-[#ef4444]/15 border-[#ef4444]/40 text-[#ef4444]") : "bg-black/4 dark:bg-white/4 border-black/10 dark:border-white/10 text-black/50 dark:text-white/50 hover:border-primary/20"}`}>
                        {t}
                      </button>
                    ))}
                  </div>
                  <div>
                    <div className="relative">
                      ...
                      <button type="button" onClick={() => aiCategorize(newEntry.description)} disabled={aiCategorizing || !newEntry.description.trim()}
                        className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1 px-2 py-1 rounded-lg bg-primary/10 hover:bg-primary/20 text-primary text-[10px] font-semibold transition-colors disabled:opacity-40">
                  ...
                  <select value={newEntry.category} onChange={(e) => setNewEntry({ ...newEntry, category: e.target.value })} className={selectCls}>
                    {CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
                  </select>
                ...
                <div className="flex gap-3 mt-6">
                  <button onClick={() => setShowAddModal(false)}
                    className="flex-1 py-2.5 rounded-xl border border-black/10 dark:border-white/10 text-black/50 dark:text-white/50 text-sm hover:text-black dark:hover:text-white transition-colors">
                    Cancel
                  </button>
        ...
        {showBudgetModal && (
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
              ...
              <div className="glass-strong border border-black/8 dark:border-white/8 rounded-2xl p-6 shadow-2xl">
                <div className="flex items-center justify-between mb-5">
                  <h3 className="text-lg font-semibold text-[#1d1d1f] dark:text-white">{editingBudget ? `Edit ${editingBudget.category}` : "Set Budget"}</h3>
                ...
                <div className="space-y-4">
                  {!editingBudget && (
                    <select value={budgetForm.category} onChange={(e) => setBudgetForm({ ...budgetForm, category: e.target.value })} className={selectCls}>
                      {BUDGET_CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
                    </select>
                  )}
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-black/40 dark:text-white/40 text-sm">$</span>
                    <input placeholder="Monthly limit" value={budgetForm.total} onChange={(e) => setBudgetForm({ ...budgetForm, total: e.target.value })}
                      type="number" min="0" step="1" className={`${inputCls} pl-8`} />
                  </div>
                </div>
                <div className="flex gap-3 mt-6">
