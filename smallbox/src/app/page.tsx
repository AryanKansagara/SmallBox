...
import { BackgroundPathsHero } from "@/components/ui/background-paths";
  ...
  return (
    <div className="min-h-screen text-[#1d1d1f] dark:text-white overflow-x-hidden">
      ...
      <nav className="glass fixed top-0 left-0 right-0 z-50 h-16 flex items-center px-6 md:px-12 border-b border-black/6 dark:border-white/6">
        ...
        <div className="flex items-center gap-3 ml-8">
          ...
          <Link href="/onboarding">
            <motion.button
              whileHover={{ scale: 1.04, boxShadow: "0 0 24px rgba(123,47,255,0.5)" }}
              whileTap={{ scale: 0.96 }}
              className="bg-primary hover:bg-primary/90 text-white text-sm font-semibold px-5 py-2 rounded-full transition-all shadow-[0_0_16px_rgba(123,47,255,0.3)]"
            >
              Get Started Free
            </motion.button>
          </Link>
      ...
      <section className="relative px-6 md:px-12 pb-24 overflow-hidden">
        <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={5} className="relative">
          <TiltCard intensity={2} className="mx-auto max-w-5xl">
            <div className="rounded-2xl border border-white/8 glass-card overflow-hidden shadow-[0_32px_80px_rgba(0,0,0,0.6)]">
              ...
              <div className="flex h-72">
                <div className="w-44 border-r border-white/6 p-3 flex flex-col gap-1">
                  {["Overview", "Website Builder", "Finance", "Marketing", "Settings"].map((item, i) => (
                    <div key={item} className={`px-3 py-2 rounded-lg text-xs font-medium ${i === 0 ? "bg-primary/12 text-primary" : "text-white/40"}`}>
                      {item}
                    </div>
                  ))}
                </div>
                <div className="flex-1 p-5 flex flex-col gap-4">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      ...
                      <div key={s.label} className="bg-white/5 rounded-xl p-3 border border-white/6">
                        <p className="text-[10px] text-white/40 mb-1">{s.label}</p>
                        <p className="text-base font-bold text-white">{s.value}</p>
                        <p className={`text-[10px] ${s.up ? "text-emerald-500" : "text-white/40"}`}>
                  ...
                  <div className="space-y-2">
                    {[
                      { label: "Budget Used", pct: 65, color: "#7b2fff" },
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {howItWorks.map((step, i) => (
              <motion.div key={step.num} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15, duration: 0.6 }} className="text-center">
                <motion.div
                  whileHover={{ scale: 1.05, boxShadow: "0 0 32px rgba(123,47,255,0.2)" }}
      ...
      <section className="py-20 px-6 md:px-12">
        <motion.div initial={{ opacity: 0, scale: 0.98 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="max-w-5xl mx-auto relative rounded-3xl overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-primary via-[#6014d4] to-[#003399]" />
      ...
      <footer className="border-t border-black/6 dark:border-white/6 py-12 px-6 md:px-12 glass">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-xl bg-primary flex items-center justify-center shadow-[0_0_12px_rgba(123,47,255,0.4)]">
